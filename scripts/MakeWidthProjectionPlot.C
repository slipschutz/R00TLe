...
/**This script is meant to take a 2-D histogram and show the width of X as a function of Y.
   For example if X is a time difference and Y is a pulse height it will show how the width
   of the time difference spectra changes with pulse height.
 ...
 */
TGraphErrors* MakeWidthProjectionPlot(TH2F* h , Double_t Start=0,Double_t End=16000,Int_t NumPoints=20,Double_t YScale=1,Double_t XScale=1,Double_t FitLowCut=-10,Double_t FitHighCut=10,TString plotG="AP",Int_t color=kBlack){
  ...
  for (int i=0;i<NumPoints;i++){
	...
	gr->SetPoint(PointCount,h->GetYaxis()->GetBinCenter(TMath::Nint((StartBin+EndBin)/2.0))/YScale,result->Value(2)*XScale);
