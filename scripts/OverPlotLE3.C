...
void OverPlotLE3(Long64_t entry=0,int BarSpot=0){
  ...
  if ( NumBottoms ==1 && NumTops == 1){
    ...
    for (int i=0;i<TraceSize;i++){
      ...
      x[i]=i;
      yTT[i]=event->Bars[0].Tops[0].GetTrace()[i];
      yTF[i]=event->Bars[0].Tops[0].GetFilter()[i];
      yTC[i]=event->Bars[0].Tops[0].GetCFD()[i];

      yBT[i]=event->Bars[0].Bottoms[0].GetTrace()[i];
      yBF[i]=event->Bars[0].Bottoms[0].GetFilter()[i];
      yBC[i]=event->Bars[0].Bottoms[0].GetCFD()[i];
